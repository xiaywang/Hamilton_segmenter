...
def find_extrema(signal=None, mode='both'):
    ...
    aux_tmp = utils.diff(signal, len(signal))
    aux_tmp1 = utils.v_sign(aux_tmp, len(aux_tmp))

    aux = utils.diff(aux_tmp1,len(aux_tmp1)) # np.diff(np.sign(np.diff(signal)))

    if mode == 'both':
        aux = utils.v_abs(aux, len(aux))
        extrema = utils.v_gtzero(aux,len(aux)) + 1
    elif mode == 'max':
        extrema = utils.v_ltzero(aux,len(aux)) + 1
    elif mode == 'min':
        extrema = utils.v_gtzero(aux,len(aux)) + 1
