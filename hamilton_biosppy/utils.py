...
def diff(v_in, N):    # not found in math.h, %% keep in mind for plplib
    v_out = np.zeros((N-1,), dtype=int)
    for i in range(N-1):
        v_out[i] = v_in[i+1] - v_in[i]
    return np.asarray(v_out)

def v_abs(v_in, N):
    v_out = np.zeros((N,), dtype=int)
    for i in range(N):
        v_out[i] = np.abs(v_in[i]) # use abs function of stdlib.h
    return np.asarray(v_out)

def v_sign(v_in, N):
    # returns 1 if element > 0, -1 if element < 0, 0 if element = 0
    v_out = np.zeros((N,), dtype=int)
    for i in range(N):
        if v_in[i] < 0:
            v_out[i] = -1
...
def v_gtzero(v_in, N):
    # return indexes of elements greater than 0
    v_out = np.zeros((N,), dtype=int)
    i = 0
    j = 0
    while i < N:
    ...
    return np.asarray(v_out[:j])

def v_nonzero(v_in, N):
    # return indexes of elements not equal to zero
    v_out = np.zeros((N,), dtype=int)
    i = 0
    j = 0
    while i < N:
