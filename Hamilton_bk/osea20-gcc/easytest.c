...
MAINTYPE main()
	...
	for(recNum = 0; recNum < REC_COUNT; ++recNum)
		...
		//printf("InputFileSampleFrequency:%d\n", InputFileSampleFrequency);
		...
		printf(" NextSample Input: ecg[0]:%d\n",ecg[0]) ;
		printf(" NextSample Input: ecg[1]:%d\n",ecg[1]) ;
		NextSample(ecg,2,InputFileSampleFrequency,SAMPLE_RATE,1) ;
		// std::cout <<"NextSample output: ecg[0]:"<<ecg[0]<<endl;
		printf(" NextSample output: ecg[0]:%d\n",ecg[0]) ;
		printf(" NextSample output: ecg[1]:%d\n",ecg[1]) ;
		//std::cout <<"NextSample output: ecg[1]:"<<ecg[1]<<endl;
		...
		//while(SampleCount < 3600)
		while(NextSample(ecg,2,InputFileSampleFrequency,SAMPLE_RATE,0) >= 0 )//&& SampleCount < 3600)
			{
			...
			//printf(" Adjust resolution: ecg[0]:%d\n",ecg[0]) ;
			//printf(" Adjust resolution: ecg[1]:%d\n",ecg[1]) ;
			// Pass sample to beat detection and classification.

			delay = BeatDetectAndClassify(ecg[0], &beatType, &beatMatch) ;
			//printf("After BeatDetectAndClassify: delay:%d, ecg[0]: %d, beatype: %d, beatmatch: %d\n", delay, ecg[0], beatType, beatMatch);
			printf("After BeatDetectAndClassify: delay:%d, ecg[0]: %d \n", delay, ecg[0]);
			...
			if(delay != 0)
				{
				//printf("Beat Detected: annot.time: %ld, SampleCount: %ld\n", DetectionTime, SampleCount);
				DetectionTime = SampleCount - delay ;
				printf("DetectionTime : %ld, ecg[0]: %d\n", DetectionTime, ecg[0] );
